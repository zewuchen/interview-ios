...
final class NetworkerTests: XCTestCase {
    ...
    func test_requestData_whenURLRequestIsNotNil_whenURLResponseIsNil_shouldGetFailure_and_shouldCallUrlSession() throws {
        let dummyEndpoint: DummyURLEndpoint = .init()
        let dummyURLRequest: URLRequest = .init(url: try XCTUnwrap(URL(string: "www.dummy.com")))
        urlSessionSpy.urlResponseToBeReturned = nil
        
        sut.requestData(endpoint: dummyEndpoint) { result in
            let result: Result<Data, NetworkerError> = result
            XCTAssertEqual(result, .failure(.unknown("HTTPURLResponse is nil")))
        }
    ...
    func test_requestData_whenURLRequestIsNotNil_whenURLResponseIsNotNil_whenStatusCodeLessThan200_shouldGetFailure_and_shouldCallUrlSession() throws {
        let dummyEndpoint: DummyURLEndpoint = .init()
        let dummyURLRequest: URLRequest = .init(url: try XCTUnwrap(URL(string: "www.dummy.com")))
        urlSessionSpy.urlResponseToBeReturned = try givemHTTPURLResponse(statusCode: 10)
    ...
    func test_requestData_whenURLRequestIsNotNil_whenURLResponseIsNotNil_whenStatusCodeGreatherThan300_shouldGetFailure_and_shouldCallUrlSession() throws {
        let dummyEndpoint: DummyURLEndpoint = .init()
        let dummyURLRequest: URLRequest = .init(url: try XCTUnwrap(URL(string: "www.dummy.com")))
        urlSessionSpy.urlResponseToBeReturned = try givemHTTPURLResponse(statusCode: 500)
    ...
    func test_requestData_whenURLRequestIsNotNil_whenURLResponseIsNotNil_whenStatusCode200_whenDataIsNil_shouldGetFailure_and_shouldCallUrlSession() throws {
        let dummyEndpoint: DummyURLEndpoint = .init()
        let dummyURLRequest: URLRequest = .init(url: try XCTUnwrap(URL(string: "www.dummy.com")))
        urlSessionSpy.urlResponseToBeReturned = try givemHTTPURLResponse(statusCode: 200)
        urlSessionSpy.dataToBeReturned = nil
    ...
    func test_requestData_whenURLRequestIsNotNil_whenURLResponseIsNotNil_whenStatusCode200_whenDataIsNotNil_shouldGetFailure_and_shouldCallUrlSession() throws {
        let dummyData: Data = try JSONEncoder().encode(DummyResponse(dummy: "dummy"))
        let dummyEndpoint: DummyURLEndpoint = .init()
        let dummyURLRequest: URLRequest = .init(url: try XCTUnwrap(URL(string: "www.dummy.com")))
        urlSessionSpy.urlResponseToBeReturned = try givemHTTPURLResponse(statusCode: 200)
        ...
        sut.requestData(endpoint: dummyEndpoint) { result in
            let result: Result<Data, NetworkerError> = result
            XCTAssertEqual(result, .success(dummyData))
        }
